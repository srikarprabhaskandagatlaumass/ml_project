...
# ----Main Execution-----
# Loading and preprocessing the rice dataset
rice_data = pd.read_csv('Dataset_3_Rice/rice.csv', header=0)

# Convert labels to binary (1 for Cammeo, 0 otherwise)
rice_data['label'] = rice_data['label'].apply(lambda x: 1 if x == 'Cammeo' else 0)

# Extract features and labels
X = rice_data.iloc[:, :-1].values  # All columns except last
y = rice_data.iloc[:, -1].values   # Last column (labels)

# Combine into single array as expected by your k-NN implementation
