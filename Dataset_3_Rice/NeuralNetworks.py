...
def generateInitialWeights(layer_sizes):
    ...
    for i in range(len(layer_sizes) - 1):
        weight_matrix = np.random.uniform(low=-1.0, high=1.0, size=(layer_sizes[i + 1], layer_sizes[i] + 1))
...
def preprocessDataset(dataset_path):
    ...
    if isinstance(y[0], str):
        y = np.array([1 if label == 'Cammeo' else 0 for label in y])
...
if __name__ == "__main__":
    case = 1 # Set to 1 for the first test case, 2 for the second test case, or None to skip test cases

    dataset_path = "Dataset_3_Rice/rice.csv" # Change this for using different datasets
